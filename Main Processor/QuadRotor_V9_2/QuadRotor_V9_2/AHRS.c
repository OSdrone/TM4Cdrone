...
void Compensacion_Sensor_magnetico(tpAHRS *AHRS){
	...
	AHRS->Orientacion_MAG = atan2(-mag_y, mag_x);
}
...
void Correccion_deriva(tpAHRS *AHRS){
	...
	//YAW
	arm_scale_f32(&AHRS->DCM_matriz[2][0], AHRS->DCM_matriz[0][0]*arm_sin_f32(AHRS->Orientacion_MAG) - AHRS->DCM_matriz[1][0]*arm_cos_f32(AHRS->Orientacion_MAG), error, 3);
